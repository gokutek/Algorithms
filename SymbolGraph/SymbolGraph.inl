inline SymbolGraph::SymbolGraph(const std::string& filename, char delimiter)
{
	...
	fs.clear();
	fs.seekg(0);
	graph_.reset(new Graphs(st_.size()));
	while (fs.getline(buffer, sizeof(buffer))) {
		std::vector<std::string> vec = StringUtils::Split(buffer, delimiter);
...
inline bool SymbolGraph::Contains(const std::string& s)
{
...
inline std::string SymbolGraph::NameOf(size_t v)
{
	assert(v < keys_.size());
	return keys_[v];
}
...
inline Graphs& SymbolGraph::GetGraph()
{
	return *graph_.get();
}
